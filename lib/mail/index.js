...
var prereleaseVerPat = /.*?-[0-9]+$/;
function createTextFromMetadata(msgs, time) {
	var packages = [];
	_.each(msgs, (m)=> {
		var versionKey =  _.keys(m.meta.versions)[0];
		// console.log('createTextFromMetadata() %j', _.keys(m.meta.versions));
		// console.log(m.meta.versions[versionKey]);
		...
		if (versionNo.indexOf('alpha') >= 0 || versionNo.indexOf('beta') >= 0 ||
			prereleaseVerPat.test(versionNo)) {
			console.log('publishing prerelease version, skip sending email for ' + m.meta.name + '@' + versionNo);
			return;
		}
		packages.push({
			name: m.meta.name,

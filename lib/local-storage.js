...
function Storage(config) {
  ...
  //TODO -- Figure out if there is some concept of default config?
  // If a store plugin hasn't been configured, put in the local-fs one.
  if(self.config.store == undefined) {
    self.config.store = {'./disk-store': 'disk'}
  }
  var store = load_plugins(config, self.config.store, {
    config : config,
    logger : self.logger,
  }, function(p) {
    return true
  })
  // TODO -- support multiple stores? I vote no. Bomb out in this case
  self.store = (store.length == 1 ? store[0] : console.log('huh? more than one store configured/found'))
...
Storage.prototype.add_package = function(name, info, callback) {
  ...
  storage.create_json(info_file, name, get_boilerplate(name), function(err) {
...
Storage.prototype.remove_package = function(name, callback) {
  ...
  storage.read_json(info_file, name, function(err, data) {
    if (err) {
      if (err.code === 'ENOENT') {
    ...
    storage.unlink(info_file, function(err) {
      ...
      unlinkNext(function() {
        ...
        storage.rmdir('.', name, function(err) {
          callback(err)
        })
      })
    })
...
Storage.prototype.update_versions = function(name, newdata, callback) {
  ...
  self._read_create_package(name, function(err, data) {
    ...
    if (change) {
      ...
      Utils.increment_package_version(data)
      self.store.write_json(info_file, name, data, function(err) {
        callback(err, data)
      })
    } else {
      callback(null, data)
...
Storage.prototype.remove_tarball = function(name, filename, revision, callback) {
  ...
  self.update_package(name, function updater(data, cb) {
    ...
    if (err) return callback(err)
    var storage = self.store
    if (storage) storage.unlink(filename, name, callback)
  })
}
...
Storage.prototype.add_tarball = function(name, filename) {
  ...
  var wstream = storage.write_stream(filename, name)
...
Storage.prototype.get_tarball = function(name, filename, callback) {
  ...
  var rstream = storage.read_stream(filename, name)
...
Storage.prototype.get_package = function(name, options, callback) {
  ...
  storage.read_json(info_file, name, function(err, result) {
    if (err) {
      if (err.code === 'ENOENT') {
        return callback( Error[404]('no such package available') )
      } else {
...
Storage.prototype.update_package = function(package, updateFn, _callback) {
  // TODO -- is this function missing. I'm not certain how the previous code we wouldn't have storage.
  // Need to do some homework to fully understand
  // if (!storage) return _callback( Error[404]('no such package available') )

  this.store.update_package(info_file, package, updateFn, this._normalize_package, _callback)
}

Storage.prototype.search = function(startkey, options) {
  console.log('local-storage.search() called')
  return 'not implemented'
}
// TODO -- Need to refactor as this is
//Storage.prototype.search = function(startkey, options) {
//  var self = this
//
...
// TODO -- should this be factored out to utils or something?

...
// Cope with https://github.com/rlidwka/sinopia/issues/67
function safe_unlink(filename, cb) {
	fs.exists(filename, function(exists){
		if( exists ) {
			var replacementFileName= filename + '.old';
			fs.rename(filename, replacementFileName, function(err) {
				if(err) { cb(err); }
				else { fs.unlink(replacementFileName, cb); }
			});
		}
		else { cb(); }
	});
